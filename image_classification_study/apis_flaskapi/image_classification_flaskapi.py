          ...
          application/json:
            schema:
              ...
              properties:
                message:
                  type: string
                  example: "Welcome to the Image Classification API!"
    """
...
# by returning the file metadata
##################################
@app.route("/test-file-upload/", methods=["POST"])
def test_file_upload():
    """
    Test file upload mechanism.
    ---
    consumes:
      - multipart/form-data
    parameters:
      - name: file
        ...
        type: file
        required: true
        description: The file to upload (JPEG image).
    responses:
      200:
            ...
            schema:
              ...
              properties:
                ...
                size:
                  ...
                  example: 12345
      400:
        description: Bad request (e.g., no file part, no selected file, or invalid file type)
      500:
...
##################################
@app.route("/predict-image-category-class-probability/", methods=["POST"])
def predict_image_category_class_probability():
    """
    Predict image category and class probabilities.
    ---
    consumes:
      - multipart/form-data
    parameters:
...
##################################
@app.route("/visualize-image-gradcam/", methods=["POST"])
def visualize_image_gradcam():
    """
    Visualize Grad-CAM heatmaps for an image.
    ---
    consumes:
      - multipart/form-data
    parameters:
    ...
    responses:
      200:
        description: Base64-encoded Grad-CAM visualization plot
        content:
          application/json:
            schema:
              type: object
